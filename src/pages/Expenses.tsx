...
import { DUMMY_EXPENSES, Expense, EXPENSE_CATEGORIES } from "@/data/dummy";
...
export default function Expenses() {
  ...
  const [form, setForm] = useState({
    category: "Art Supplies", description: "", amount: "", method: "UPI" as const,
    date: new Date().toISOString().slice(0, 10),
  });

  const filtered = activeCategory === "All" ? expenses : expenses.filter(e => e.category === activeCategory);
  const total = expenses.reduce((a, e) => a + e.amount, 0);
  const monthTotal = expenses.filter(e => e.date.startsWith("2024-02")).reduce((a, e) => a + e.amount, 0);
  ...
  const addExpense = () => {
    ...
    setExpenses(prev => [...prev, { ...form, id, amount: Number(form.amount) }]);
    setShowForm(false);
